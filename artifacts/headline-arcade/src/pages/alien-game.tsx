...
function drawUFOPlayer(ctx: CanvasRenderingContext2D, x: number, y: number, vy: number, shrug: boolean, t: number) {
  ...
  ctx.fillStyle = "#5a6a7a"; ctx.strokeStyle = "#00f7c0"; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.ellipse(0, 24 + saucerBob, 28, 9, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  // saucer dome tint
  ctx.fillStyle = "rgba(0,247,192,0.15)";
  ctx.beginPath(); ctx.ellipse(0, 24 + saucerBob, 28, 9, 0, 0, Math.PI * 2); ctx.fill();
  // lights on saucer
...
function drawPipe(ctx: CanvasRenderingContext2D, pipe: Pipe, canvasH: number, t: number) {
  ...
  // CLASSIFIED stamp on top pipe
  ctx.fillStyle = "rgba(200,20,20,0.85)";
  const stampH = 26;
  ctx.beginPath(); ctx.roundRect(pipe.x + 3, Math.max(5, topH - 60), PIPE_W - 6, stampH, 3); ctx.fill();
...
function drawFolder(ctx: CanvasRenderingContext2D, f: Folder, t: number) {
  ...
  const bob = Math.sin(t * 0.1 + f.x) * 4;
  ctx.translate(0, bob);
  // glow
  ...
  ctx.textAlign = "center"; ctx.textBaseline = "middle";
  ctx.fillText("ALIEN", 0, -2);
  ctx.fillText("FILES", 0, 7);
  ctx.restore();
}
...
export default function AlienGame() {
  ...
  const G = useRef({
    py: 200, vy: 0,
    pipes: [] as Pipe[], folders: [] as Folder[],
    particles: [] as Particle[],
    score: 0, pipeTimer: 0,
    shrug: false, shrugT: 0,
  ...
  const spawnParticles = (x: number, y: number) => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      g.particles.push({ x, y, vx: Math.cos(a) * spd, vy: Math.sin(a) * spd, life: 40, color: ["#ff4444", "#ffaa00", "#ffdd00"][Math.floor(Math.random() * 3)] });
    }
  };
  ...
  useEffect(() => {
    ...
    const loop = (time: number) => {
      ...
      if (g.pipeTimer >= 100) {
        ...
        const pipeX = W + 10;
        g.pipes.push({ x: pipeX, gapY, scored: false });
        // folder at gap center
        g.folders.push({ x: pipeX + PIPE_W / 2, y: gapY, collected: false });
      }
      ...
      for (const p of g.pipes) {
        if (!p.scored && p.x + PIPE_W < g.playerX) { p.scored = true; }
      }
      ...
      if (g.py < 20 || g.py > H - 20) {
        spawnParticles(g.playerX, g.py);
        g.shrug = true; g.shrugT = 80; g.running = false;
        setTimeout(endGame, 1400);
      }

      // collision: pipes (forgiving hitbox)
      for (const p of g.pipes) {
        const inX = g.playerX > p.x + 6 && g.playerX < p.x + PIPE_W - 6;
        const inTopPipe = g.py < p.gapY - PIPE_GAP / 2 + 10;
        const inBotPipe = g.py > p.gapY + PIPE_GAP / 2 - 10;
    ...
    const draw = (ctx: CanvasRenderingContext2D, W: number, H: number, g: typeof G.current) => {
      ...
      for (let i = 0; i < 30; i++) {
        const sx = (i * 137.5 + g.t * 0.2) % W;
        const sy = (i * 79.3) % (H * 0.7);
        const r = 0.5 + (i % 3) * 0.5;
        ctx.beginPath(); ctx.arc(sx, sy, r, 0, Math.PI * 2); ctx.fill();
      }

      for (const p of g.pipes) drawPipe(ctx, p, H, g.t);
      ...
      if (g.shrug && g.shrugT > 0) {
        ctx.save();
        const alpha = Math.min(1, g.shrugT / 20);
      ...
      {phase === "over" && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/85">
          <div className="text-center px-6 max-w-md w-full">
            ...
            <p className="text-white/60 text-sm mb-4">The files remain classified. For now.</p>
            <p className="text-[#00f7c0] text-4xl font-black mb-1">{scoreDisplay}</p>
            <p className="text-white/40 text-xs mb-8">Files Declassified · Best: {hs}</p>
